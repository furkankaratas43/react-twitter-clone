...
export default function Search(){
    ...
    return(
        <div ref={ref} className="min-h-[32px] h-[53px] mb-3 flex items-center relative">
            <label className="h-[43px] bg-[#202327] rounded-full w-full group border border-transparent relative focus-within:bg-black focus-within:border-[#1d9bf0]">
               <div className="w-14 h-full flex items-center justify-center absolute top-0 left-0">
                ...
                <input
                    ...
                    value={query}
                    type="text" placeholder="Ara" className="w-full h-full bg-transparent rounded-full outline-none pl-14 text-[15px] placeholder-[#71767b]" />
                {(query && focus) && (
                    <button type={"button"} onClick={() => setQuery('')} className="w-[22px] h-[22px] rounded-full bg-[#1d9bf0] flex items-center justify-center text-black min-w-[22px] right-3 absolute top-1/2 -translate-y-1/2">
            ...
            {focus && (
                <div className="absolute top-full w-[350px] min-h-[100px] -left-px -translate-y-1 bg-black rounded-lg shadow-box max-h-[calc(80vh-53px)] text-center">
                    <p className="p-3 pt-5 text-[#71767b] leading-5">Kişileri, listeleri veya anahtar kelimeleri aramayı dene</p>
                </div>
            )}
        </div>
    )
