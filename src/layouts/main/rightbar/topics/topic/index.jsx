...
export default function Topic({item}){
    return(
        <Link className="py-3 px-4 transition-colors hover:bg-[color:var(--background-third)] relative" to="/">
            <div className="text-[0.813rem] text-[color:var(--color-base-secondary)] leading-4">{item.title}</div>
            <div className="font-bold mt-0.5 leading-5">{item.topic.type === 'tag' && '#'}{item.topic.value}</div>
            {item?.postCount && (
                <div className="text-[0.813rem] text-[color:var(--color-base-secondary)] mt-1 leading-4">{formatter(item.postCount)} posts</div>
            )}
            <Popover className="absolute top-1.5 right-2">
                <Popover.Button
                    type="button"
                    className="outline-none w-[34.75px] h-[34.75px] text-[color:var(--color-base-secondary)] rounded-full flex justify-center items-center hover:text-[color:var(--color-primary)] transition-colors relative before:absolute before:inset-0 before:rounded-full hover:before:hover:bg-[color:var(--color-primary)] before:transition-all before:z-[-1] before:opacity-[.15] z-[1]">
                ...
                <Popover.Panel className="w-[24rem] grid absolute top-2 right-0 bg-[color:var(--background-primary)] overflow-hidden shadow-box rounded-xl z-[2]">
                    <button type="button" className="hover:bg-[color:var(--background-secondary)] transition-colors text-left py-3 px-4 flex items-center gap-3 font-bold leading-5">
                        <svg viewBox="0 0 24 24" className="h-[1.172rem]">
